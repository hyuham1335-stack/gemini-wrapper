...
컨텍스트와 토큰은 유한한 자원입니다. 아래 규칙은 "정확도를 낮춰 대충 하라"는 뜻이 아니라
**같은 결과를 더 적은 읽기·출력으로 얻으라**는 뜻입니다. 검증 루프(L1/L2)나 절대 규칙을
토큰 절약을 이유로 건너뛰지 마세요 — 그 둘은 예산보다 우선합니다.

**읽기 (가장 큰 낭비 지점)**

- 파일 전체 읽기는 기본적으로 금지. `Grep`으로 위치를 먼저 잡고 `Read`의 `offset`/`limit`으로
  필요한 구간만 읽습니다 (특히 300줄 이상 파일).
- 같은 파일을 두 번 읽지 않습니다. 편집 결과 확인용 재읽기 금지 — 실패했다면 Edit/Write가 에러를 냅니다.
- 통째로 읽지 않는 것: `package-lock.json`, `node_modules/**`, `.next/**`,
  `lib/supabase/database.types.ts`(생성물), `supabase/migrations/` 전량. 필요한 심볼만 grep하세요.
- 스키마·데이터 확인은 파일 덤프보다 Supabase MCP `list_tables` / 좁은 `execute_sql`이 훨씬 쌉니다.

**명령 출력**

- 장황한 출력은 반드시 필터해서 실행합니다:
  `npx tsc --noEmit 2>&1 | head -40`, `npm run build 2>&1 | tail -30`,
  `git diff --stat` 먼저 → 필요한 파일만 `git diff -- <path>`.
- `npm install` / `npm run dev` 로그 전문을 컨텍스트에 올리지 않습니다 (dev 서버는 백그라운드로
  띄우고 필요한 줄만 확인).
- 실패한 명령은 원인을 고쳐 재실행하되, 동일 명령을 3회 넘게 반복하지 않습니다 (검증 루프 규칙과 동일).

**탐색·위임**

- 서브에이전트는 사용자가 요청했을 때만 사용합니다. 다만 "여러 디렉터리를 훑어야 답이 나오는 탐색"은
  Explore에 맡기는 편이 총 토큰이 싼 경우가 있으니, 그때는 먼저 사용자에게 제안하고 승인받으세요.
- 이미 대화에서 확인한 사실(파일 위치, 스키마, 플랜 한도 등)은 다시 조회하지 않고 재사용합니다.

**경고 임계값 (self-check)**

아래 중 하나라도 걸리면 **작업을 멈추고 사용자에게 한 줄로 보고한 뒤** 계속할지 확인합니다.
자동으로 강제되지 않으므로 스스로 세고, 넘었으면 조용히 넘어가지 말고 말하세요.

| 신호 | 임계값 | 보고 예시 |
| --- | --- | --- |
| 컨텍스트 잔량 | 30% 미만 | "컨텍스트가 30% 미만입니다. 남은 작업 X를 이어갈지, 요약 후 새 세션으로 갈지 알려주세요." |
| 한 요청에서 읽은 파일 수 | 15개 초과 | "탐색이 예상보다 커졌습니다(파일 N개). 범위를 좁힐까요?" |
| 한 요청의 도구 호출 수 | 25회 초과 | "같은 목표로 N회 호출했습니다. 접근을 바꾸거나 범위를 나눌까요?" |
| 단일 도구 출력 | 대략 500줄 초과 | 필터를 걸어 다시 실행하고, 왜 컸는지 한 줄 보고 |

- 탐색이 컸던 작업(위 임계값 중 하나라도 근접)은 보고 마지막에 "읽은 파일 N개 / 명령 M회" 한 줄을
  남겨 사용자가 비용을 가늠할 수 있게 합니다. 작은 작업에는 붙이지 마세요.
